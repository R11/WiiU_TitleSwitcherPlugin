...
// Grid dimensions must match actual OSScreen character grid on hardware
// OSScreen uses a fixed character grid regardless of pixel resolution
// These values are from src/render/renderer.cpp (OS_SCREEN_COLS/ROWS)
inline const ScreenConfig SCREEN_CONFIGS[] = {
    // DRC (GamePad) - 854x480, OSScreen uses 100x18 char grid
    { "DRC (GamePad)", 854, 480, 100, 18, 8, 24, false },
    // TV 1080p - 1920x1080 (16:9), larger grid for higher res
...
// Based on hardware observation, the divider appears at ~45% of screen width
inline int GetDividerCol() {
    return (sConfig->gridCols * 45) / 100;
}
