/**
 * Bitmap Font for Colored Text Rendering
 *
 * 8x8 pixel monospace font covering ASCII printable characters (32-126).
 * Each character is stored as 8 bytes, one per row, with bits representing pixels.
...
// 8x8 bitmap font data - classic PC/console style
// Each character is 8 bytes (one per row), MSB is leftmost pixel
static const uint8_t FONT_DATA[CHAR_COUNT][8] = {
    // 32: Space
 ...
 * @return Pointer to 8 bytes of glyph data, or nullptr if out of range
 */
 ...
 * @param glyph Pointer to 8 bytes of glyph data
 * @param x X position within character (0-7, 0 is left)
 * @param y Y position within character (0-7, 0 is top)
 * @return true if pixel is set
 */
inline bool IsPixelSet(const uint8_t* glyph, int x, int y) {
    if (!glyph || x < 0 || x >= CHAR_WIDTH || y < 0 || y >= CHAR_HEIGHT) {
        return false;
    }
